...
def process_list_to_tiles(image_num_list, create_features=CREATE_FEATURES):
    """
    ...
    for slide_num in image_num_list:
        ...
        np_img_array = np.array(df['np_img'].values.tolist())
        BATCH_SIZE = 64
        dataset = tf.data.Dataset.from_tensor_slices(np_img_array).batch(BATCH_SIZE)
        features_list = []
        for batch in dataset
            features = intermediante_layer_model.predict(batch)[0]
...
def process_range_to_tiles(start_index, end_index, create_features=CREATE_FEATURES):
    """
