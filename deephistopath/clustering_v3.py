    ...
    with tqdm(total = len(tile_indices), desc = pbar_info, position = slide_num) as pbar:
        for t in tile_indices:
            ...
            tile = Tile(tile_sum, slide_num, count, r, c, r_s, r_e, c_s, c_e, o_r_s, o_r_e, o_c_s,
                        o_c_e, t_p, t_p_score, np_scaled_tile)
...
def process_list_to_tiles(image_num_list):
    ...
    for slide_num in image_num_list:
        ...
        tile_sum_list.append(tile_summary.to_dict())
...
def singleprocess_filter_and_features(start_index, image_num_list=None):
    ...
    if image_num_list is not None:
        tile_sum_list = process_list_to_tiles(image_num_list)
    else:
        num_training_slides = get_num_training_slides()
        tile_sum_list = process_range_to_tiles(start_index, start_index + num_training_slides - 1)

    columns = ['ID', 'filename', 'orig_w', 'orig_h', 'orig_tile_w', 'orig_tile_h', 'scaled_w', 'scaled_h',
               'scaled_tile_w', 'scaled_tile_h', 'tissue_percentage', 'num_col_tiles', 'num_row_tiles']
    tile_sum_info = pd.DataFrame(index=[], columns=columns)
    for tile_sum_i in tile_sum_list:
        tile_sum_info.append(tile_sum_i, ignore_index=True)
    tile_sum_info.to_pickle(os.path.join(DEST_DataFrame_DIR, "summary" + ".pkl"))
...
def multiprocess_filter_and_features(image_num_list=None):
    ...
    for result in results:
        tile_summary_info = result.get()
        tile_sum_list.extend(tile_summary_info)
...
class TileSummary:
    ...
    def to_dict(self):
        return {
            'ID': TRAIN_PREFIX + str(self.slide_num).zfill(3),
...
class Tile:
    ...
    def __init__(self, tile_summary, slide_num, tile_num, r, c, r_s, r_e, c_s, c_e, o_r_s, o_r_e,
                 o_c_s, o_c_e, tissue_percentage, t_p_score, np_scaled_tile):
